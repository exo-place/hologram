...
- `rest-timeout.ts` — Discord REST fetch timeout. `installRestTimeout(bot.rest)` patches Discordeno's `createRequestBody` so every REST request carries an `AbortSignal.timeout` (30 s default; 60 s for file uploads). Without it a black-holed connection pends forever and permanently wedges Discordeno's serial per-route queue — the failure mode where all message-path REST awaits (deputy check, channel/guild metadata) hang until the channel-queue timeout while `/trigger` still works. Installed once in `client.ts` right after `createBot`.
