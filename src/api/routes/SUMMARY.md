...
- **bindings.ts** — Discord binding CRUD: list bindings with optional filters (`GET /api/bindings?discord_id=&discord_type=&entity_id=`), add a binding (`POST /api/bindings`), remove a binding by row id (`DELETE /api/bindings/:id`). Wraps `addDiscordEntity`/`listDiscordMappings`/`clearPersonaCache` from `db/discord.ts`.
