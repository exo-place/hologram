...
/** Create a fully-initialised in-memory DB mirroring what getDb() does. */
...
/**
 * Mirror of initSchema() from src/db/index.ts.
 * Kept in sync with the production schema so we test what ships.
 */
function applySchema(db: Database) {
  ...
  db.exec(`DELETE FROM messages WHERE discord_message_id IS NOT NULL AND id NOT IN (SELECT MIN(id) FROM messages WHERE discord_message_id IS NOT NULL GROUP BY discord_message_id)`);
  ...
  // Migrations
  try { db.exec(`ALTER TABLE entities ADD COLUMN system_template TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE entities ADD COLUMN config_thinking TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE entities ADD COLUMN config_collapse TEXT`); } catch { /* already exists */ }
