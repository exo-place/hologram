...
- `entities.ts` — Entity and fact CRUD: `createEntity`, `getEntity`, `updateEntity`, `deleteEntity`, `transferOwnership`, `setEntityNickname`, `addFact`, `updateFactByContent`, `removeFactByContent`, `setFacts`, `searchEntities`. Also exposes `getEntityConfig`/`setEntityConfig`, per-entity eval defaults (`getEntityEvalDefaults`), `getEntityKeywords` (parses `config_keywords` into a string array for trigger keyword matching), and `safeParseFallback` (safe `JSON.parse` with a fallback, used throughout for corrupted DB data). The `Entity` interface includes a `nickname` field (nullable) used for autocomplete disambiguation.
