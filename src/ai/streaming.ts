...
export async function* handleMessageStreaming(
  ...
  const imageModelSpec = entities[0]?.imageModelSpec ?? null;
  ...
  try {
    ...
    for (let attempt = 0; attempt <= 1; attempt++) {
      ...
      const result = useTools
        ? streamText({ ...baseStreamOptions, tools: createTools(channelId, guildId, ctx.triggerEntityFn, imageOptions, entities[0]?.id, entities[0]?.ownedBy) })
      ...
      // Merge sidecar files (from generate_image tool calls)
      generatedFiles.push(...sidecarFiles);
