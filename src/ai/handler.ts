...
function parseMultiCharacterResponse(
  ...
  type ParsedResponse = CharacterResponse & { position: number; streamMode?: "lines" | null };
...
export interface StreamingContext extends MessageContext {
  /** Entity to stream for (single entity only) */
  entity: EvaluatedEntity;
}

/**
 * Handle a message with line-based streaming.
 * Yields complete lines as they're generated by the LLM.
 * Only supports single-entity responses (no multi-character XML parsing).
 */
export async function* handleMessageStreaming(
  ctx: StreamingContext
): AsyncGenerator<string, void, unknown> {
  const { channelId, guildId, entity } = ctx;

  // Build context similar to handleMessage
  const other: EntityWithFacts[] = [];
  const seenIds = new Set([entity.id]);

  // Add location entity if referenced
  const locationPattern = /^is in \[entity:(\d+)\]/;
  const locIndex = entity.facts.findIndex(f => locationPattern.test(f));
  if (locIndex !== -1) {
    const match = entity.facts[locIndex].match(locationPattern);
  ...
  // Add item entities if referenced
  const itemPattern = /^has \[entity:(\d+)\]/;
  for (let i = 0; i < entity.facts.length; i++) {
    const match = entity.facts[i].match(itemPattern);
    if (match) {
  ...
  // Build prompts
  const systemPrompt = buildSystemPrompt([entity], other);
  const userMessage = buildUserMessage(
    history.slice().reverse().map(m => ({ author_name: m.author_name, content: m.content }))
  );
  ...
  try {
    ...
    // Stream text and yield complete lines
    for await (const delta of result.textStream) {
      ...
      while ((newlineIndex = buffer.indexOf("\n")) !== -1) {
        ...
        // Skip empty lines and <none/> sentinel
        if (line && line !== "<none/>") {
          yield line;
        }
      }
    ...
    // Process tool results after streaming
