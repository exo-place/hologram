...
- **@discordeno/bot unpinned `-next` risk** — the installed build (`22.0.1-next.758e75f` per node_modules) drifts from the pin above, and `-next` builds can regress without notice. Two REST failure modes wedged the serial route queues (2026-06/07 incidents: mention/random-reply dead while `/trigger` worked): (1) `fetch` with no timeout/AbortSignal hangs forever on a black-holed connection; (2) after a socket-level failure (`FailedToOpenSocket`) the queue's rate-limit bookkeeping (`remaining`/`interval`/`frozenAt` in queue.js — never updated via response headers on the error path) can stall `processPending` so no fetch is ever issued (observed 2026-07-06→08: ~50 task timeouts, zero fetch aborts). Mitigated by `src/bot/rest-timeout.ts`: layer 1 patches `createRequestBody` to attach `AbortSignal.timeout`; layer 2 patches `makeRequest` with a 90s outer promise timeout that also drops the stalled route queue from `rest.queues`. Re-verify both hook points (`createRequestBody` → `new Request(url, payload)` → `fetch`; queue key `${authorization}${simplifyUrl(route, method)}` in `processRequest`) whenever the discordeno version changes; move to a stable release when v22 ships.
- **File an upstream issue on @discordeno/rest** — queue state after socket failures: `sendRequest`'s fetch-error path rejects the request but never restores the queue's rate-limit bookkeeping (no headers processed → `remaining`/`interval`/`frozenAt` left inconsistent), which can stall `processPending` permanently. Also: `fetch` is called with no timeout/AbortSignal. Reproduced in production 2026-06-24 and 2026-07-06.
